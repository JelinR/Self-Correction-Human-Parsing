...
from .backbone.resnest_utils.splat import SplAtConv2d, DropBlock2D
import math
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)       #Shape: (8, 128, 64, 64)
        x = self.bn1(x)        
        x = self.relu(x)
        x = self.maxpool(x)     #Shape: (8, 128, 32, 32)

        x2 = self.layer1(x)     #Shape: (8, 256, 32, 32)
        x3 = self.layer2(x2)    #Shape: (8, 512, 16, 16)
        x4 = self.layer3(x3)    #Shape: (8, 1024, 16, 16)
        x5 = self.layer4(x4)    #Shape: (8, 2048, 16, 16)
        ...
        # Edge Branch
        edge_result, edge_fea = self.edge(x2, x3, x4)       #Shapes: (8, 2, 32, 32), (8, 768, 32, 32)
        
        # Fusion Branch
        x = torch.cat([parsing_fea, edge_fea], dim=1)       #Shape: (8, 1024, 32, 32). Here, 1024 is (3+1)*256.

        fusion_result = self.fushion(x)                     #Shape: (8, 20, 32, 32)

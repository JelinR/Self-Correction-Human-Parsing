...
class LiteHRNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            num_channels = self.stages_spec['num_channels'][i]                      #A tuple of number of channels for this stage. Eg: (40, 80)
            num_channels = [num_channels[i] for i in range(len(num_channels))]      #Converts to list of numbers (copy)
            setattr(
                self, 'transition{}'.format(i),
                self._make_transition_layer(num_channels_last, num_channels))       #Assigns a transition layer to each stage
    ...
    def _make_transition_layer(self, num_channels_pre_layer,
        ...
        num_branches_cur = len(num_channels_cur_layer)      #Number of branches in current stage
        num_branches_pre = len(num_channels_pre_layer)      #Number of branches in previous stage

        transition_layers = []
        for i in range(num_branches_cur):       #Iterates through current number of branches

            #If the current branch is a continuation of the previous parallel branches. 
            # That is, if there were previously 2 branches and now 3 branches, this equality is 
            # valid when we are dealing with the first two branches (not the new third one).
            if i < num_branches_pre:        

                #If the same branch in the current and previous stages has the same number of channels, 
                # then there is no transition. We use the input as the output. Else, there is a transition.                                    
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:          
                    transition_layers.append(
                        nn.Sequential(
                            build_conv_layer(                           #3x3 Depthwise separable convolution
                                self.conv_cfg,
            ...
            else:                                                       #If the transition is to a new additional branch (2->3)
                conv_downsamples = []
                for j in range(i + 1 - num_branches_pre):               #Remember that i (current branch number) has to greater than the number of previous branches.
                    in_channels = num_channels_pre_layer[-1]            #TODO Doubt: Why is this choosing the last branch's channels at all times? 
    ...
    def forward(self, x):
        ...
        # return [x[0]]
        return x                                #TODO CHANGED: return all branch outputs

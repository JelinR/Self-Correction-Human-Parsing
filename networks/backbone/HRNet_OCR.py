...
from .hrnet_utils.hrnet_ocr_utils import BatchNorm2d, BatchNorm2d_class, relu_inplace
...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsampling
        x0_h, x0_w = x[0].size(2), x[0].size(3)                         #Shape of x[0]: (8, 48, 32, 32)
        x1 = F.interpolate(x[1], size=(x0_h, x0_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)   #Shape: (8, 96, 16, 16) -> (8, 96, 32, 32)
        x2 = F.interpolate(x[2], size=(x0_h, x0_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)   #Shape: (8, 192, 8, 8) -> (8, 192, 32, 32)
        x3 = F.interpolate(x[3], size=(x0_h, x0_w),
                        mode='bilinear', align_corners=ALIGN_CORNERS)   #Shape: (8, 384, 4, 4) -> (8, 384, 32, 32)
        ...
        #TODO ADDED
        out_aux_seg.append(x[1])
        out_aux_seg.append(x[2])
        out_aux_seg.append(x[3])
